...
## Step 3: Test the Administrator Module

### A. Manage Specializations & Doctors
1. In the sidebar, click on **Specializations**.
2. Click **+ Add Specialization**. Enter "Cardiologist" and save.
3. Click on **Doctor Profiles** in the sidebar.
4. Click **+ Add Doctor Profile**. Select an existing staff account and the "Cardiologist" specialization. Set their Consultation Fee (e.g., `500`) and save.

### B. Add a Receptionist User
*To test the Receptionist module, we need a Receptionist account!*
...
### C. View System Roles
1. In the sidebar, click on **System Roles** to verify you can see the read-only list of all active roles in the system.
...
4. Select the **Doctor** (created in Step 3A) and the **Date**.
...
7. Click the **Edit** (pencil) icon next to the bill. Change the Additional Charges to `75` and click Update. Verify the Total Amount accurately updates!
8. Click the **View Receipt** (document) icon to generate a printable, formatted HTML bill window.

---

## Step 5: Test the Doctor Module

*To test this module, you must use the Doctor account you linked in Step 3A. Ensure you use the Receptionist account to book a 'Scheduled' appointment with that specific Doctor.*
...
### C. Patient History Lookup
1. Click **Patient History** in the sidebar.
2. Enter the Patient ID of the patient you just diagnosed and click **Fetch History**.
3. Verify that the Consultation timeline, Medicine Prescriptions, and Lab Test Results are rendered in chronological order!
...
1. Click **Medicine Categories** in the sidebar.
2. Click **+ Add Category**. Enter a name like "Antibiotics" and save.
...
6. **Verification:** 
   ...
   - A **Print Medicine Bill** HTML window should automatically open containing the receipt.
...
1. Click **Dictionary Management** in the sidebar.
2. Click **+ Add Lab Test**.
3. Define the Test Name, Cost, Min/Max Reference Ranges, and Sample Required. Save the test.
4. Click **Lab Tests Reference** in the sidebar to view the active dictionary of all available lab tests.
